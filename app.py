...
def carregar_dados_generico(tabela, user_id):
    try:
        ...
        # Correção: Garante que o DF tenha colunas mesmo se vazio para evitar KeyError
        if df.empty:
            return pd.DataFrame(columns=['id', 'descricao', 'valor', 'user_id', 'created_at'])
        return df
    ...
    try:
        ...
        if not df.empty:
            df['valor'] = pd.to_numeric(df['valor'])
             ...
             # Retorna estrutura vazia para evitar erro no editor
             return pd.DataFrame(columns=['id', 'data', 'descricao', 'valor', 'categoria', 'tipo', 'user_id'])
        return df
...
def executar_sql(tabela, acao, dados, user_id):
    try:
        ...
        if acao == 'insert':
            # Remove ID se for nulo/nan para deixar o banco gerar
...
with st.sidebar:
    ...
    with st.expander("🔄 Gastos Fixos/Recorrentes"):
        ...
        if st.button("Salvar Fixos"):
            # Lógica de deleção SEGURA (Correção do KeyError)
            ...
            # Identificação de IDs deletados com segurança
            if not edit_recorrente.empty and 'id' in edit_recorrente.columns:
    ...
    with st.expander("🎯 Metas & Sonhos"):
        ...
        if st.button("Salvar Metas"):
            ...
            # CORREÇÃO PRINCIPAL: Verificação segura de colunas
...
# =======================================================
    ...
    if not df_total.empty:
        ...
        if st.button("💾 Atualizar Extrato", type="primary"):
            ids_orig = df_edit['id'].tolist() if not df_edit.empty else []
            
            for i, row in mudancas.iterrows():
                d = row.to_dict()
                if isinstance(d.get('data'), (date, datetime)): d['data'] = d['data'].strftime('%Y-%m-%d')
                if pd.isna(d.get('id')): pass 
                else: executar_sql('transactions', 'update', d, user['id'])
            
            # Verificação segura de deleção
            if not mudancas.empty and 'id' in mudancas.columns:
